...
public class B implements Runnable {
    ...
    public static void main(String[] args) {
        new Thread(null, new B(), String.join(
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            ...
            int[][] segments = in.nextIntMatrix(segmentCount, 3);

            int[] spending = getSpending(segmentCount, segments);
            for (var coins : spending) {
                out.println(coins);
            }
        }
    ...
    private static int[] getSpending(int segmentCount, int[][] segments) {
        int[] spending = new int[segmentCount];

        int min = Integer.MAX_VALUE;
        int max = Integer.MIN_VALUE;
        int minCost = 1_000_000_000;
        int maxCost = 1_000_000_000;
        int comboCost = Integer.MAX_VALUE; // Both min and max
        for (int i = 0; i < segmentCount; i++) {
            if (segments[i][0] < min) {
                min = segments[i][0];
                minCost = segments[i][2];
                comboCost = Integer.MAX_VALUE;
            } else if (segments[i][0] == min) {
                minCost = Math.min(minCost, segments[i][2]);
            }

            if (segments[i][1] > max) {
                max = segments[i][1];
                maxCost = segments[i][2];
                comboCost = Integer.MAX_VALUE;
            } else if (segments[i][1] == max) {
                maxCost = Math.min(maxCost, segments[i][2]);
            }

            if (segments[i][0] == min && segments[i][1] == max) {
                comboCost = Math.min(comboCost, segments[i][2]);
            }

            spending[i] = Math.min(minCost + maxCost, comboCost);
        }
