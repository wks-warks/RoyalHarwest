...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            ...
            setResult(0, 0, k, result, initial, moves);

            out.println(result.toString());
            out.printArray(moves);
        }
        
        out.close();
    }

    private static void setResult(int idx, int flipTotal, int flipLimit, StringBuilder result, String initial, int[] moves) {
        char currBit = (char) (initial.charAt(idx) ^ (flipTotal & 1));
        if (idx + 1== moves.length) {
            int flips = flipLimit - flipTotal;
            moves[idx] = flips;
            
            result.append(currBit);
        } else {
            int flips = 0;

            if (flipTotal == flipLimit) {
                result.append(currBit);
            } else {
                int remainder = flipLimit - flipTotal;

                flips = 1 ^ ((currBit ^ remainder) & 1);
                moves[idx] = flips;
                
                char newBit = (char) (currBit ^ ((remainder - flips) & 1));
                result.append(newBit);
            }

            setResult(idx + 1, flipTotal + flips, flipLimit, result, initial, moves);
        }
    }
}
