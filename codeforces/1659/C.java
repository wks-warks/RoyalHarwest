...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            ...
            long[] ps = getPrefixSum(x);
        
            long cost = getCost(n, a, b, x, ps);
            out.println(cost);
        }
    ...
    private static long getCost(int n, int a, int b, int[] x, long[] ps) {
        long cost = b * ps[n-1];

        long conqueredCost = (long) b * x[0];
        for (int i = 0; i < n; i++) {
            long movingCost = (long) a * x[i];
            long netRemainingDistance = ps[n-1] - ps[i] - (long) x[i] * (n - 1 - i);
            long toConquerCost = b * netRemainingDistance;
            long netCost = movingCost + conqueredCost + toConquerCost;
            cost = Math.min(cost, netCost);

            if (i + 1 < n) {
                conqueredCost += (long) b * (x[i+1] - x[i]);
            }
        }
