...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            int[] particles = in.nextIntArray(in.nextInt());

            int harmoniousLength = getHarmoniousLength(particles);
            out.println(harmoniousLength);
        }
        
        out.close();
    }

    private static int getHarmoniousLength(int[] particles) {
        int harmoniousLength = -1;

        Map<Integer, Integer> lastOccurrence = new HashMap<Integer, Integer>();
        for (int i = 0; i < particles.length; i++) {
            int type = particles[i];
            if (lastOccurrence.containsKey(type)) {
                int lo = lastOccurrence.get(type);

                int candidateLength = lo + particles.length - i;
                harmoniousLength = Math.max(harmoniousLength, candidateLength);
            }
            lastOccurrence.put(type, i);
        }
