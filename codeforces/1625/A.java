...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            int formCount = in.nextInt();
            int letters = in.nextInt();

            int[] forms = in.nextIntArray(formCount);
            int initial = getInitial(forms, formCount);
            out.println(initial);
        }
    ...
    private static int getInitial(int[] forms, int formCount) {
        int[] freq = new int[31];
        
        for (var form : forms) {
            updateFreq(freq, form);
        }

        int initial = 0;
        for (int i = 0; i <= 30; i++) {
            int oneCount = freq[i];
            int zeroCount = formCount - oneCount;
    ...
    private static void updateFreq(int[] freq, int form) {
        for (int i = 0; i <= 30; i++) {
            if ((form & (1<<i)) > 0) {
                freq[i]++;
            }
