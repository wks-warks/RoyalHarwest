...
fn main() {
    ...
    for _ in 0..tests {
        ...
        for _ in 0..n {
            a.push(scan.next_usize());
        }

        let crossings = count_crossings(n, a);
        writeln!(out, "{}", crossings).ok();
    }
}

fn count_crossings(n: usize, a: Vec<usize>) -> u64 {
    let mut ft = FenwickTree::with_size(n + 1);
    
    let mut crossings = 0;
    for seg in a {
        crossings += ft.sum(seg as isize, n as isize) as u64;
        ft.add(seg, 1);
    }
...
impl FenwickTree {
    fn with_size(n: usize) -> FenwickTree {
        let ft = vec![0; n];
        FenwickTree { ft }
    }

    fn sum_from_start(&mut self, mut r: isize) -> usize {
        let mut sum = 0;
        while r >= 0 {
            sum += self.ft[r as usize];
            r = FenwickTree::g(r) - 1;
        }
        sum
    }

    fn sum(&mut self, left: isize, right: isize) -> usize {
        self.sum_from_start(right) - self.sum_from_start(left - 1)
    }
