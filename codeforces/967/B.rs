...
fn main() {
    ...
    let a = scan.next_usize();
    let b = scan.next_usize();
    let mut s = Vec::with_capacity(n);
    for _ in 0..n {
        s.push(scan.next_usize());
    }

    let block_holes = count_block_holes(a, b, s);
    writeln!(out, "{}", block_holes).ok();
}

fn count_block_holes(a: usize, b: usize, mut s: Vec<usize>) -> usize {
    let first = s[0];
    s.remove(0);
    s.sort();
    
    let mut running_sum = first;
    for i in 0..s.len() {
        running_sum += s[i];
        
        if first * a < running_sum * b {
            return s.len() - i;
        }
    }
