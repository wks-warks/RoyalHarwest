...
public class Main implements Runnable {
    ...
    private static String getP(String s) {
        if (s.charAt(0) == '9') {
            // target: 111..111
            // 111 - ab = (100 - ab + 11) = 9-a + 10-b + 11
            StringBuilder target = new StringBuilder();
            for (int i = 0; i <= s.length(); i++) {
                target.append('1');
            }
            BigInteger bi = new BigInteger(target.toString());
            BigInteger sbi = new BigInteger(s);
            return bi.subtract(sbi).toString();
        } else {
            StringBuilder pSb = new StringBuilder();
            for (var ch : s.toCharArray()) {
                char pCh = (char) ('0' + ('9' - ch));
                pSb.append(pCh);
            }
            return pSb.toString();
        }
