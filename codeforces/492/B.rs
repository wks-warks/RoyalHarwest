...
fn main() {
    ...
    let lanterns = scan.next_u16();
    let road_length = scan.next_u32();
    let mut lantern_pos = Vec::new();
    for _ in 0..lanterns {
        lantern_pos.push(scan.next_u32());
    }

    let radius = get_radius(lantern_pos, road_length);
    writeln!(out, "{}", radius).ok();
}

fn get_radius(mut lantern_pos: Vec<u32>, road_length: u32) -> f64 {
    lantern_pos.sort();
    let mut radius: f64 = std::cmp::max(*lantern_pos.first().unwrap(), road_length - lantern_pos.last().unwrap()) as f64;

    for (i, curr) in lantern_pos.iter().enumerate() {
        if i + 1 < lantern_pos.len() {
