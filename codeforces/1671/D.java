...
public class Main implements Runnable {
    ...
    static long getCost(int x, int[] a) {
        if (a.length == 1) {
            return Math.max(x, a[0]) - 1;
        }

        long cost = 0;
        int largest = Integer.MIN_VALUE;
        for (int i = 1; i < a.length; i++) {
            int lesser = Math.min(a[i], a[i-1]);
            int greater = Math.max(a[i], a[i-1]);

            cost += greater - lesser;
            largest = Math.max(largest, greater);
        }
    ...
    static int getMinCost(int num, int[] a) {
        int cost = Math.min(
            Math.abs(a[0] - num),
            Math.abs(a[a.length-1] - num)
        );

        for (int i = 1; i < a.length; i++) {
            cost = Math.min(cost, Math.abs(num - a[i]) + Math.abs(num - a[i-1]) - Math.abs(a[i] - a[i-1]));
        }
