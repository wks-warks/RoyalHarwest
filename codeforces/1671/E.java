...
public class Main implements Runnable {
    ...
    static int countWays(String s) {
        ...
        String[] sortedPreorder = new String[s.length()];

        for (int i = s.length()-1; i > 0; i--) {
            ways[i] = 1;
            sortedPreorder[i] = "" + s.charAt(i);

            if ((i << 1) < s.length()) {
                ways[i] = (int) ((long) ways[i<<1] * ways[i<<1|1] % MOD);

                int cmp = sortedPreorder[i<<1].compareTo(sortedPreorder[i<<1|1]);
                if (cmp != 0) {
                    ways[i] <<= 1;
                    ways[i] %= MOD;
                }
                ...
                if (cmp <= 0) {
                    sortedPreorder[i] += sortedPreorder[i<<1] + sortedPreorder[i<<1|1];
                } else {
                    sortedPreorder[i] += sortedPreorder[i<<1|1] + sortedPreorder[i<<1];
                }
            }
        }
