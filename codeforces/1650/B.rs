...
fn main() {
    ...
    for _ in 0..tests {
        let (left, right, div) = (scan.next_u32(), scan.next_u32(), scan.next_u32());
        let max = get_max_f(left, right, div);

        writeln!(out, "{}", max).ok();
    }
}

fn get_max_f(left: u32, right: u32, div: u32) -> u32 {
    if div == 1 {
        return right;
    } else if left / div == right / div {
        return right / div + right % div;
    }else if right % div == div - 1 {
        return right / div + div - 1;
    } else {
        return right / div + div - 2;
    }
}
