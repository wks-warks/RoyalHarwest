...
fn main() {
    ...
    for _ in 0..tests {
        ...
        set_result(a, &mut result);
        result.reverse();

        for shift in result.into_iter() {
            write!(out, "{} ", shift).ok();
        }
        writeln!(out).ok();
    }
...
fn set_result(mut a: Vec<usize>, result: &mut Vec<usize>) {
    if a.len() == 0 {
        return;
    }

    let idx = get_idx(&a, a.len());
    let shift = if idx == a.len() - 1 { 0 } else { idx + 1 };
    result.push(shift);

    a.remove(idx);
    let len = a.len();
    if shift > 0 {
        a.rotate_left(shift - 1);
    }

    set_result(a, result);
}

fn get_idx(a: &Vec<usize>, num: usize) -> usize {
