...
public class Main implements Runnable {
    ...
    static long countMoves(int[] a) {
        ...
        for (int i = 0; i < a.length; i++) {
            long candidate = countBackward(i, a) + countForward(i, a);
            moves = Math.min(moves, candidate);
        }

        return moves;
    }

    static long countBackward(int i, int[] a) {
        long moves = 0;
        long prevHt = 0;
        for (int j = i-1; j >= 0; j--) {
            long addition = LIG(prevHt + 1, a[j]);
            prevHt = a[j] * addition;
            moves += addition;
        }
        return moves;
    ...
    static long countForward(int i, int[] a) {
        long moves = 0;
        long prevHt = 0;
