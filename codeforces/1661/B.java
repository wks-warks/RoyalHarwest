...
public class Main implements Runnable {
    ...
    private static final int TWO_POW = 32_768;
    static Map<Integer, Integer> memo = new HashMap<Integer, Integer>();
    static {
        fillMemo();
    }
    private static int[] countOps(int[] a) {
    ...
    private static void fillMemo() {
        ...
        while (!queue.isEmpty()) {
            ...
            Integer child = getChild(top.x - 1);
            if (child != null) {
                memo.put(child, top.y + 1);
                queue.add(new Point(child, top.y + 1));
            }

            if ((top.x & 1) == 0) {
                child = getChild(top.x >> 1);
                if (child != null) {
                    memo.put(child, top.y + 1);
                    queue.add(new Point(child, top.y + 1));
                }

                child = getChild((top.x + TWO_POW) >> 1);
                if (child != null) {
                    memo.put(child, top.y + 1);
                    queue.add(new Point(child, top.y + 1));
                }
    ...
    private static Integer getChild(int num) {
        if (num < 0) {
            num += TWO_POW;
        }
