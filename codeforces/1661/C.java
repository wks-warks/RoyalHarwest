...
public class Main implements Runnable {
    ...
    private static long countDays(int[] h) {
        int maxHeight = 0;
        for (var height : h) {
            maxHeight = Math.max(height, maxHeight);
        }

        return Math.min(countDays(h, maxHeight), countDays(h, maxHeight + 1));
    }

    private static long countDays(int[] h, int maxHeight) {
        long left = 0;
        long right = 10_000_000_000_000_000L;

        long days = right;
        while (left <= right) {
            long mid = (left + right) >> 1;
            if (isPossible(mid, h, maxHeight)) {
                days = mid;
                right = mid - 1;
            } else {
                left = mid + 1;
    ...
    private static boolean isPossible(long days, int[] h, int maxHeight) {
        long even = days >> 1;
        long odd = days - even;
        ...
        for (var height : h) {
            long diff = ((maxHeight - height) >> 1) << 1;
            if (even > 0) {
                long useEven = Math.min(even, diff >> 1);
                even -= useEven;
                diff -= useEven << 1;
            }
