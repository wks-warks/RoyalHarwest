...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int t = 0; t < tests; t++) {
            ...
            Tree tree = new Tree(n, p);
            int time = tree.computeTime();
            out.println(time);
        }
...
class Tree {
    ...
    Tree(int n, int[] p) {
        ...
        for (int i = 0; i < p.length; i++) {
            nodes[i + 1].setParent(nodes[p[i] - 1]);
            nodes[p[i] - 1].addChild(nodes[i + 1]);
        }
    }

    int computeTime() {
        PriorityQueue<Integer> uninfectedSets = new PriorityQueue<Integer>(nodes.length, Collections.reverseOrder());
        for (var node : nodes) {
            int children = node.children.size();

            if (children > 0) {
                uninfectedSets.add(children);
            }
        }

        int infected = 1;
        int time = uninfectedSets.size() + 1;
        PriorityQueue<Integer> infectedSets = new PriorityQueue<Integer>(Collections.reverseOrder());
        while (!uninfectedSets.isEmpty()) {
            int reduction = uninfectedSets.size() + 1;
            int children = uninfectedSets.poll();

            infected += Math.min(children, reduction);
            if (children > reduction) {
                infectedSets.add(children - reduction);
            }
        }

        while (infected < nodes.length) {
            int top = infectedSets.peek();

            if (top == 1) {
                time++;
                infected = nodes.length;
                break;
            } else {
                int half = top >> 1;
                time += half;
                ...
                while (!infectedSets.isEmpty()) {
                    int children = infectedSets.poll();

                    if (children > half) {
                        next.add(children - half);
                        infected += half;
                    } else {
                        infected += children;
                    }
