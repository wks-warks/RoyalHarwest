...
public class Main implements Runnable {
    ...
    private static int countOps(int[] a, int n) {
        ...
        return countOps(maxCount, n);
    }

    private static int countOps(int maxCount, int n) {
        if (maxCount == n) {
            return 0;
        } else {
            int addition = Math.min(n - maxCount, maxCount);
            return 1 + addition + countOps(maxCount + addition, n);
        }
    }
}
