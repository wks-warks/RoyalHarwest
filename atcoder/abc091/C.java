...
//         new Thread(null, new A(), String.join(
...
public class Main implements Runnable {
    ...
    public void run() {
        int n = in.nextInt();
        int[][] red = in.nextIntMatrix(n, 2);
        int[][] blue = in.nextIntMatrix(n, 2);

        int friendlyPairs = countFriendlyPairs(red, blue);
        out.println(friendlyPairs);
        
        out.close();
    }

    private static int countFriendlyPairs(int[][] red, int[][] blue) {
        TreeMap<Integer, Integer> availableRed = new TreeMap<Integer, Integer>();
        ...
        int bPtr = 0;
        int friendlyPairs = 0;
        while (bPtr < blue.length) {
            if (rPtr < red.length && red[rPtr][0] < blue[bPtr][0]) {
                insert(availableRed, red[rPtr++][1]);
            } else {
                friendlyPairs += fetch(availableRed, blue[bPtr++][1]);
            }
        }
    ...
    private static int cmp(int[] p1, int[] p2) {
    ...
    private static void insert(TreeMap<Integer, Integer> tm, int key) {
        Integer prev = tm.get(key);
        if (prev == null) {
            prev = 0;
        }
        tm.put(key, prev + 1);
    ...
    private static int fetch(TreeMap<Integer, Integer> tm, int limit) {
        Integer lower = tm.lowerKey(limit);
        if (lower == null) {
            return 0;
        }

        int prev = tm.get(lower);
        if (prev == 1) {
            tm.remove(lower);
        } else {
            tm.put(lower, prev - 1);
