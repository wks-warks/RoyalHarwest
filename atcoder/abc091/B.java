...
public class Main implements Runnable {
    ...
    public void run() {
        String[] positive = getStrings(in.nextInt());
        String[] negative = getStrings(in.nextInt());

        int earning = getEarning(positive, negative);
        out.println(earning);
    ...
    private int getEarning(String[] positive, String[] negative) {
        Map<String, Integer> map = new HashMap<String, Integer>();
        populate(map, positive, 1);
        populate(map, negative, -1);

        int earning = 0;
        for (var value : map.values()) {
            earning = Math.max(earning, value);
        }

        return earning;
    }

    private static void populate(Map<String, Integer> map, String[] arr, int addition) {
        for (var str : arr) {
            Integer prev = map.get(str);
            if (prev == null) {
                prev = 0;
            }
            map.put(str, prev + addition);
        }
    }
}