...
fn main() {
    ...
    let (n, k) = (scan.next_usize(), scan.next_usize());
    let s = scan.next_string().chars().collect::<Vec<char>>();

    let fs = get_fs(s, k);
    writeln!(out, "{}", fs).ok();
}

fn get_fs(s: Vec<char>, k: usize) -> usize {
    for i in 1..s.len() {
        if s.len() % i == 0 && is_possible(&s, k, i) {
            return i;
        }
    }
    s.len()
...
fn is_possible(s: &Vec<char>, k: usize, i: usize) -> bool {
    let mut freq = vec![vec![0; 26]; i];

    for (idx, ch) in s.iter().enumerate() {
        freq[idx % i][*ch as usize - 'a' as usize] += 1;
    }
