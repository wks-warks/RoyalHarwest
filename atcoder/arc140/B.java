...
public class Main implements Runnable {
    ...
    private static int countOps(String s) {
        int[] trailingA = new int[s.length()];
        for (int i = 0; i < s.length() - 1; i++) {
            if (s.charAt(i) == 'A') {
                trailingA[i + 1] = trailingA[i] + 1;
            }
        }

        int[] followingC = new int[s.length()];
        for (int i = s.length() - 1; i > 0; i--) {
            if (s.charAt(i) == 'C') {
                followingC[i - 1] = followingC[i] + 1;
            }
        }
        
        NavigableMap<Integer, Integer> nmap = new TreeMap<Integer, Integer>();
        for (int i = 0; i < s.length(); i++) {
            if (s.charAt(i) == 'R') {
                increase(nmap, Math.min(trailingA[i], followingC[i]));
            }
        }
        ...
        int moves = 0;
        boolean oddMove = true;
        while (!nmap.isEmpty()) {
            moves += 1;

            if (oddMove) {
                Integer largest = nmap.lastKey();
                reduce(nmap, largest);
                if (largest > 1) {
                    increase(nmap, largest - 1);
                }
            } else {
                Integer smallest = nmap.firstKey();
                reduce(nmap, smallest);
            }
    ...
    private static void increase(NavigableMap<Integer, Integer> nmap, int key) {
        Integer prev = nmap.get(key);
        if (prev == null) {
            prev = 0;
        }
        nmap.put(key, prev + 1);
    ...
    private static void reduce(NavigableMap<Integer, Integer> nmap, int key) {
        Integer prev = nmap.get(key);
        if (prev == 1) {
            nmap.remove(key);
        } else {
