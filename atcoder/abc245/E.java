...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[] d = in.nextIntArray(m);

        boolean possible = isPossible(a, b, c, d, n, m);
        out.println(possible ? "Yes" : "No");
        
        out.close();
    }

    private static boolean isPossible(int[] a, int[] b, int[] c, int[] d, int n, int m) {
        PriorityQueue<Point> chocolates = new PriorityQueue<Point>(n, (x, y) -> (x.x == y.x ? y.y - x.y : y.x - x.x));
        for (int i = 0; i < n; i++) {
            chocolates.add(new Point(a[i], b[i]));
        }

        PriorityQueue<Point> boxes = new PriorityQueue<Point>(m, (x, y) -> (x.x == y.x ? y.y - x.y : y.x - x.x));
        for (int i = 0; i < m; i++) {
            boxes.add(new Point(c[i], d[i]));
        }

        NavigableMap<Integer, Integer> boxHeights = new TreeMap<Integer, Integer>();
        
        while (!chocolates.isEmpty()) {
            Point chocolate = chocolates.poll();
            
            while (!boxes.isEmpty()) {
                if (boxes.peek().x < chocolate.x) {
                    break;
                }

                Point box = boxes.poll();
                Integer prev = boxHeights.get(box.y);
                if (prev == null) {
                    prev = 0;
                }
                boxHeights.put(box.y, prev + 1);
            }

            Integer ceiling = boxHeights.ceilingKey(chocolate.y);
            if (ceiling == null) {
                return false;
            } else if (boxHeights.get(ceiling) == 1) {
                boxHeights.remove(ceiling);
            } else {
                boxHeights.put(ceiling, boxHeights.get(ceiling) - 1);
            }
        }
