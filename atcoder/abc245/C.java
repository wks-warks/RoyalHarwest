...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        boolean possible = isPossible(a, b, k, n);
        out.println(possible ? "Yes" : "No");
        
        out.close();
    }

    private static boolean isPossible(int[] a, int[] b, int k, int n) {
        boolean aPossible = true;
        boolean bPossible = true;

        for (int i = 1; i < n; i++) {
            boolean nextAPossible = Math.min(aPossible ? Math.abs(a[i] - a[i-1]) : Integer.MAX_VALUE, bPossible ? Math.abs(a[i] - b[i-1]) : Integer.MAX_VALUE) <= k;
            boolean nextBPossible = Math.min(aPossible ? Math.abs(b[i] - a[i-1]) : Integer.MAX_VALUE, bPossible ? Math.abs(b[i] - b[i-1]) : Integer.MAX_VALUE) <= k;
            aPossible = nextAPossible;
            bPossible = nextBPossible;
        }
