...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int minChanges = getMinChanges(s);
    ...
    private static int getMinChanges(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            sum[i+1] = sum[i] + (s.charAt(i) == 'W' ? -1 : 1);
        }
        sum[s.length() + 1] = sum[s.length()];

        int minChanges = Integer.MAX_VALUE;
        for (int i = 1; i <= s.length(); i++) {
            int changes = computeChanges(sum, i);
    ...
    private static int computeChanges(int[] sum, int i) {
        int before = i - 1;
        int after = sum.length - 2 - i;
        
        int swapBefore = (before - sum[before]) >> 1;
        int swapAfter = (after + (sum[sum.length-1] - sum[i])) >> 1;
