...
public class Main implements Runnable {
    ...
    public void run() {
        // Make subsets with indices and numbers and find answer. DFS
        int n = in.nextInt();
        int m = in.nextInt();
        int[] p = new int[n];
        for (int i = 0; i < n; i++) {
        ...
        int covered = getCovered(p, edges);
        out.println(covered);
        
        out.close();
    }

    private static int getCovered(int[] p, int[][] edges) {
        int covered = 0;
        ...
        for (int i = 0; i < p.length; i++) {
            ...
            visited[i] = true;
            populate(i, p, visited, neighbours, positions, numbers);
            for (int e : positions) {
    ...
    private static void populate(int i, int[] p, boolean[] visited, List<List<Integer>> neighbours, Set<Integer> positions, Set<Integer> numbers) {
        numbers.add(p[i]);
        for (var pos : neighbours.get(i)) {
            positions.add(pos);
            if (!visited[pos]) {
                visited[pos] = true;
                populate(pos, p, visited, neighbours, positions, numbers);
            }
        }
    }
}
