...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int largestPerfect = getPerfect(x);
        out.println(largestPerfect);

        out.close();
    }

    private static int getPerfect(int x) {
        boolean[] perfectArr = getPerfectArr(x);
        for (int i = x; i >= 1; i--) {
            if (perfectArr[i]) {
                return i;
            }
    ...
    private static boolean[] getPerfectArr(int x) {
        boolean[] perfectArr = new boolean[x + 1];
        perfectArr[1] = true;
