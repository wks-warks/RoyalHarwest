...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        String res = getRes(num);
        out.println(res);        

        out.close();
    }

    private static String getRes(String num) {
        int[] sum = new int[num.length()];
        sum[0] = num.charAt(0) - '0';

        for (int i = 1; i < sum.length; i++) {
            sum[i] = sum[i-1] + (num.charAt(i) - '0');
        }
    ...
    private static String getRes(int[] sum) {
        int idx = sum.length-1;
        StringBuilder res = new StringBuilder();

        long curr = 0;
        while (idx >= 0 || curr != 0) {
            int pres = idx >= 0 ? sum[idx--] : 0;
            curr += pres;

            char digit = (char) ('0' + curr % 10);
            res.append(digit);
            curr /= 10;
        }
