...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int b = 0; b < bagCount; b++) {
            bags[b] = in.nextIntArray(in.nextInt());
        }

        int ways = countWays(1, 0, bags, desired);
        out.println(ways);
    ...
    private static int countWays(long current, int bagIdx, int[][] bags, long desired) {
        if (bagIdx == bags.length) {
            return current == desired ? 1 : 0;
        }

        int count = 0;
        for (var num : bags[bagIdx]) {
            if (current > desired / num) {
                continue;
            }
            count += countWays(current * num, bagIdx+1, bags, desired);
        }
