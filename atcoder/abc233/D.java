...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        long count = countSatisfactorySubseq(seq, k);
        out.println(count);
        
        out.close();
    }

    private static long countSatisfactorySubseq(int[] seq, long desiredSum) {
        long count = 0;
        long[] prefixSum = getPrefixSum(seq);

        Map<Long, Integer> sumFreq = new HashMap<Long, Integer>();
        for (var sum : prefixSum) {
            long diff = sum - desiredSum;

            int freq = getValue(sumFreq, diff);
            count += freq;

            int prev = getValue(sumFreq, sum);
            sumFreq.put(sum, prev + 1);
        }

        return count;
    }

    private static int getValue(Map<Long, Integer> map, long key) {
        Integer prev = map.get(key);
        return prev == null ? 0 : prev;
    }

    private static long[] getPrefixSum(int[] seq) {
        long[] prefixSum = new long[seq.length + 1];
