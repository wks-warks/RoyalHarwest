...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        long answer = getAnswer(n, st, x, y);
        out.println(answer);

        out.close();
    }

    private static long getAnswer(int n, SparseTable st, int x, int y) {
        long answer = 0;

        for (int i = 0; i < n; i++) {
            int minIdx = getMinIdx(i, n, st, x, y);
            int maxIdx = getMaxIdx(i, n, st, x, y);

            if (Math.min(minIdx, maxIdx) > -1) {
                answer += maxIdx - minIdx + 1;
            }
        }
    ...
    private static int getMinIdx(int i, int n, SparseTable st, int x, int y) {
        int j = -1;
    ...
    private static int getMaxIdx(int i, int n, SparseTable st, int x, int y) {
        int j = -1;
...
class SparseTable {
    ...
    SparseTable(int[] a) {
        minST = new int[a.length][20];
        maxST = new int[a.length][20];
        lg = new int[a.length + 1];
        ...
        for (int j = 1; j < 20; j++) {
            for (int i = 0; i + (1 << j) <= minST.length; i++) {
    ...
    int min(int left, int right) {
        ...
        return Math.min(minST[left][j], minST[right - (1<<j) + 1][j]);
    }

    int max(int left, int right) {
        int j = lg[right - left + 1];
        return Math.max(maxST[left][j], maxST[right - (1<<j) + 1][j]);
    }
}
