...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        boolean possible = isPossible(names);
        out.println(possible ? "Yes" : "No");
        
        out.close();
    }

    private static boolean isPossible(String[][] names) {
        boolean possible = true;

        for (int i = 0; i < names.length; i++) {
            possible &= isPossible(names, i, 0) || isPossible(names, i, 1);
        }

        return possible;
    }

    private static boolean isPossible(String[][] names, int i, int j) {
        for (int k = 0; k < names.length; k++) {
            if (k == i) {
                continue;
            }

            if (names[i][j].equals(names[k][0]) || names[i][j].equals(names[k][1])) {
                return false;
            }
        }
