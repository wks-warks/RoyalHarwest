...
class Cylinder {
    Deque<Pair<Integer, Integer>> cylinder;

    Cylinder() {
        cylinder = new ArrayDeque<Pair<Integer, Integer>>();
    }

    void add(int x, int c) {
        Pair<Integer, Integer> addition = new Pair<Integer, Integer>(x, c);
        cylinder.add(addition);
    }
    ...
    long remove(int c) {
        ...
        while (c > 0) {
            Pair<Integer, Integer> top = cylinder.pollFirst();
            int take = Math.min(c, top.getSecond());
            c -= take;

            sum += (long) top.getFirst() * take;

            top.setSecond(top.getSecond() - take);
            if (top.getSecond() > 0) {
                cylinder.addFirst(top);
            }
        }
