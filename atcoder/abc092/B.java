...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int chocolates = countChocolates(d, x, a);
        out.println(chocolates);

        out.close();
    }

    private static int countChocolates(int days, int remaining, int[] periods) {
        int chocolates = remaining;

        for (var period : periods) {
            chocolates += 1 + (days - 1) / period;
        }
