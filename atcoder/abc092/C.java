...
public class Main implements Runnable {
    ...
    public void run() {
        int[] pos = in.nextIntArray(in.nextInt());
        int[] costs = getCosts(pos);
    ...
    private static int[] getCosts(int[] pos) {
        int[] priorCosts = getCosts(pos, 0, pos.length, 1);
        int[] subsequentCosts = getCosts(pos, pos.length-1, -1, -1);

        int[] costs = new int[pos.length];
        for (int i = 0; i < pos.length; i++) {
            int priorCost = i == 0 ? 0 : priorCosts[i - 1];
            int subsequentCost = i == pos.length - 1 ? 0 : subsequentCosts[i + 1];
            
            int prevPos = i == 0 ? 0 : pos[i - 1];
            int nextPos = i == pos.length - 1 ? 0 : pos[i + 1];
            costs[i] = Math.abs(nextPos - prevPos) + priorCost + subsequentCost;
        }

        return costs;
    }

    private static int[] getCosts(int[] pos, int start, int end, int increment) {
        int prev = 0;
        int[] costs = new int[pos.length];

        for (int i = start; i != end; i += increment) {
            costs[i] = (i == start ? 0 : costs[i - increment]) + Math.abs(pos[i] - prev);
            prev = pos[i];
        }
