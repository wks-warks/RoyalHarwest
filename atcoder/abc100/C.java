...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int operations = computeOperations(n, a);
        out.println(operations);
        
        out.close();
    }

    private static int computeOperations(int n, int[] a) {
        int traversals = countTraversals(a);
        return traversals;
    }

    private static int countTraversals(int[] a) {
        int traversals = 0;
        for (var num : a) {
            while ((num > 0) && ((num & 1) == 0)) {
                num >>= 1;
