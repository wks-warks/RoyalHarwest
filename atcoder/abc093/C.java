...
public class Main implements Runnable {
    ...
    public void run() {
        int[] nums = in.nextIntArray(3);
        int operations = countOperations(nums);
    ...
    private static int countOperations(int[] nums) {
        ...
        if (nums[0] == nums[2]) {
            ...
            int individual = (nums[1] - nums[0] + 1) >> 1;
            nums[0] += individual << 1;
            return individual + countOperations(nums);
        }

        int diff = nums[2] - nums[1];
        nums[0] += diff;
        nums[1] += diff;
        return diff + countOperations(nums);
    }
}