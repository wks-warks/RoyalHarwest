...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        List<List<Integer>> occurrences = getOccurrences(a);
        ...
        for (int q = 0; q < queries; q++) {
            ...
            int count = getCount(left, right, occurrences.get(num));
            out.println(count);
        }
    ...
    private static List<List<Integer>> getOccurrences(int[] a) {
        List<List<Integer>> occurrences = new ArrayList<List<Integer>>(a.length + 1);
    ...
    private static int getCount(int left, int right, List<Integer> occurrences) {
        int start = getStart(left, occurrences);
        int end = getEnd(right, occurrences);

        if (Math.min(start, end) == -1 || start > end) {
            return 0;
        } else {
            return end - start + 1;
        }
    ...
    private static int getStart(int left, List<Integer> occurrences) {
        int lower = 0;
        int upper = occurrences.size() - 1;
        int start = -1;

        while (lower <= upper) {
            int mid = (lower + upper) >> 1;
            if (occurrences.get(mid) >= left) {
                start = mid;
                upper = mid - 1;
            } else {
                lower = mid + 1;
    ...
    private static int getEnd(int right, List<Integer> occurrences) {
        int lower = 0;
        int upper = occurrences.size() - 1;
        int end = -1;
