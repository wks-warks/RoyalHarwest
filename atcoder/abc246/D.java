...
public class Main implements Runnable {
    ...
    private static long getX(long n) {
        long x = Long.MAX_VALUE;

        for (int a = 0; a <= 1_000_000; a++) {
            int b = getB(a, n);
            x = Math.min(polynomial(a, b), x);
        }

        return x;
    }

    private static int getB(int a, long n) {
        int left = 0;
        int right = 1_000_000;
    ...
    private static long polynomial(long a, long b) {
        return  a * a * a
              + a * a * b
              + a * b * b 
              + b * b * b;
