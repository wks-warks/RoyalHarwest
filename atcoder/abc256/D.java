...
public class Main implements Runnable {
    ...
    public void run() {
        int n = in.nextInt();
        int[][] ranges = in.nextIntMatrix(n, 2);

        Map<Integer, Integer> minimumRanges = getMinimumRanges(ranges);
        for (var entry : minimumRanges.entrySet()) {
    ...
    private static Map<Integer, Integer> getMinimumRanges(int[][] ranges) {
        NavigableMap<Integer, Integer> rangeMap = new TreeMap<Integer, Integer>();
        for (var range : ranges) {
            if (rangeMap.containsKey(range[0])) {
                rangeMap.put(range[0], Math.max(range[1], rangeMap.get(range[0])));
            } else {
                rangeMap.put(range[0], range[1]);
            }
        }

        int start = -1;
        int end = -1;
        Map<Integer, Integer> minimumRanges = new TreeMap<Integer, Integer>();
