...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int ways = tree.countWaysToPaint();
        out.println(ways);
...
class Tree {
    ...
    void connect(int x, int y) {
        ...
        nodes[y].connect(nodes[x]);
    }

    int countWaysToPaint() {
        int blackRootWays = nodes[0].countWaysToPaint(null, 1);
        int whiteRootWays = nodes[0].countWaysToPaint(null, 0);
        return (blackRootWays + whiteRootWays) % Main.MOD;
    }
}
...
class Node {
    ...
    Integer whiteWays, blackWays;

    Node (int idx) {
        this.idx = idx;
        neighbours = new ArrayList<Node>();
        whiteWays = blackWays = null;
    }
    ...
    int countWaysToPaint(Node parent, int color) {
        if (color == 0 && whiteWays != null) {
            return whiteWays;
        }
        if (color == 1 && blackWays != null) {
            return blackWays;
        }
        ...
        for (var nbr : neighbours) {
            if (!nbr.equals(parent)) {
                int subtreeWays = nbr.countWaysToPaint(this, 0);
                subtreeWays %= Main.MOD;
                if (color != 1) {
                    subtreeWays += nbr.countWaysToPaint(this, 1);
                    subtreeWays %= Main.MOD;
                }
                
                ways = (int) ((long) ways * subtreeWays % Main.MOD);
            }
        }

        return color == 0 ? ( whiteWays = ways) : (blackWays = ways);
    }

    @Override
    public boolean equals(Object o) {
        return o instanceof Node && (((Node) o).idx == this.idx);
    }
}