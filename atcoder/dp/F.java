...
public class Main implements Runnable {
    ...
    private static String getLCS(String first, String second) {
        int[][][] lcsData = new int[first.length()+1][second.length()+1][3];

        for (int fIdx = 0; fIdx < first.length(); fIdx++) {
            for (int sIdx = 0; sIdx < second.length(); sIdx++) {
                int currLength = lcsData[fIdx][sIdx][0];
                int prevFIdx = lcsData[fIdx][sIdx][1];
                int prevSIdx = lcsData[fIdx][sIdx][2];

                if (first.charAt(fIdx) == second.charAt(sIdx)) {
                    update(lcsData, currLength+1, fIdx, sIdx, fIdx+1, sIdx+1);
                }

                update(lcsData, currLength, prevFIdx, prevSIdx, fIdx+1, sIdx);
                update(lcsData, currLength, prevFIdx, prevSIdx, fIdx, sIdx+1);
            }
        }

        int[] lcsEnd = new int[2];
        for (int fIdx = 0; fIdx <= first.length(); fIdx++) {
            for (int sIdx = 0; sIdx <= second.length(); sIdx++) {
                if (lcsData[fIdx][sIdx][0] > lcsData[lcsEnd[0]][lcsEnd[1]][0]) {
                    lcsEnd[0] = fIdx;
                    lcsEnd[1] = sIdx;
                }
            }
        ...
        while (lcsData[lcsEnd[0]][lcsEnd[1]][0] != 0) {
            int f = lcsData[lcsEnd[0]][lcsEnd[1]][1];
            int s = lcsData[lcsEnd[0]][lcsEnd[1]][2];
            lcsEnd[0] = f;
            lcsEnd[1] = s;
            lcs.append(first.charAt(lcsEnd[0]));
        }

        return lcs.reverse().toString();
    }

    private static void update(int[][][] lcsData, int candLength, int candFIdx, int candSIdx, int updFIdx, int updSIdx) {
        if (candLength > lcsData[updFIdx][updSIdx][0]) {
            lcsData[updFIdx][updSIdx][0] = candLength;
            lcsData[updFIdx][updSIdx][1] = candFIdx;
            lcsData[updFIdx][updSIdx][2] = candSIdx;
        }
    }
}