...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[] heights = in.nextIntArray(stones);

        int costToEnd = computeCostToEnd(heights);
        out.println(costToEnd);

        out.close();
    }

    private static int computeCostToEnd(int[] heights) {
        ...
        for (int i = 0; i < costs.length; i++) {
            if (i+1 < costs.length) {
                int addition = Math.abs(heights[i+1] - heights[i]);
                costs[i+1] = Math.min(costs[i+1], costs[i] + addition);
            }
            if (i+2 < costs.length) {
                int addition = Math.abs(heights[i+2] - heights[i]);
                costs[i+2] = Math.min(costs[i+2], costs[i] + addition);
            }
        }
