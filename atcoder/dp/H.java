...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        char[][] grid = in.nextCharMatrix(height, width);

        int differentPaths = countDifferentPaths(grid, 1_000_000_007);
        out.println(differentPaths);
        
        out.close();
    }

    private static int countDifferentPaths(char[][] grid, int mod) {
        int[][] paths = new int[grid.length][grid[0].length];
        paths[0][0] = 1;
        
        for (int i = 0; i < paths.length; i++) {
            for (int j = 0; j < paths[i].length; j++) {
                update(paths, i, j, mod, grid, i+1, j);
                update(paths, i, j, mod, grid, i, j+1);
            }
        }
    ...
    private static void update(int[][] paths, int currI, int currJ, int mod, char[][] grid, int toI, int toJ) {
        if (toI < grid.length && toJ < grid[toI].length && grid[toI][toJ] != '#') {
            int currPaths = paths[currI][currJ];
            int nextPaths = paths[toI][toJ] + currPaths;
            paths[toI][toJ] = nextPaths % mod;
        }
    }
}