...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[][] itemDetails = in.nextIntMatrix(items, 2);

        long maxValue = computeMaxValue(capacity, itemDetails);
        out.println(maxValue);
        
        out.close();
    }

    private static long computeMaxValue(int capacity, int[][] itemDetails) {
        long[] minWeight = new long[100_001];
        for (int value = 1; value < minWeight.length; value++) {
            minWeight[value] = capacity<<1;
        }

        for (var details : itemDetails) {
            int weight = details[0];
            int value = details[1];
            
            for (int netValue = minWeight.length-1; netValue >= value; netValue--) {
                long candidate = minWeight[netValue-value] + weight;
                minWeight[netValue] = Math.min(minWeight[netValue], candidate);
            }
        }

        int maxValue = 0;
        for (int value = 1; value < minWeight.length; value++) {
            if (minWeight[value] <= capacity) {
                maxValue = value;
            }
        }
        return maxValue;
