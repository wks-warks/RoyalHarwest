...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int jumpLimit = in.nextInt();
        int[] heights = in.nextIntArray(stones);

        int costToEnd = computeCostToEnd(heights, jumpLimit);
        out.println(costToEnd);
        
        out.close();
    }

    private static int computeCostToEnd(int[] heights, int jumpLimit) {
        ...
        for (int i = 0; i < costs.length; i++) {
            for (int j = 1; j <= jumpLimit; j++) {
                if (i+j == costs.length) {
                    break;
                }

                int addition = Math.abs(heights[i+j] - heights[i]);
                costs[i+j] = Math.min(costs[i+j], costs[i] + addition);
            }
        }
