...
public class Main implements Runnable {
    ...
    public void run() {
        int vacationDuration = in.nextInt();
        int[][] happinessBoost = in.nextIntMatrix(vacationDuration, 3);

        int maximumHappiness = computeMaximumHappiness(happinessBoost);
        out.println(maximumHappiness);
        
        out.close();
    }

    private static int computeMaximumHappiness(int[][] happinessBoost) {
        int[][] netHappiness = new int[happinessBoost.length][3];
        for (int act = 0; act < 3; act++) {
            netHappiness[0][act] = happinessBoost[0][act];
        }

        int end = netHappiness.length-1;
        for (int day = 0; day < end; day++) {
            for (int currAct = 0; currAct < 3; currAct++) {
                for (int nextAct = 0; nextAct < 3; nextAct++) {
                    ...
                    int candidate = netHappiness[day][currAct] + happinessBoost[day+1][nextAct];
                    netHappiness[day+1][nextAct] = Math.max(netHappiness[day+1][nextAct], candidate);
                }
            }
        }
        ...
        for (int act = 0; act < 3; act++) {
            maximumHappiness = Math.max(maximumHappiness, netHappiness[end][act]);
        }
