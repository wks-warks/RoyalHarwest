...
public class Main implements Runnable {
    ...
    public void run() {
        int n = in.nextInt();
        int[][] compatible = in.nextIntMatrix(n, n);

        int waysToPair = countWaysToPair(n, compatible);
        out.println(waysToPair);
        
        out.close();
    }

    private static final int MOD = 1_000_000_007;
    private static int countWaysToPair(int n, int[][] compatible) {
        int[] ways = new int[1 << n];
        ways[0] = 1;

        /*
            We pair the men in order.
            Therefore, if there is one pair, man_0 is part of it.
            If there are two pairs, man_0 and man_1 are part of it.
            If there are k pairs, man_0, man_1, ...m man_k-1 are part of it.
        
            ways[mask] stores number of ways to have matched women in the mask to the first BitCount(mask) men.
        */
        ...
        for (int mask = 0; mask < ways.length; mask++) {
            for (int w = 0; w < n; w++) {
                ...
                if ((mask & wbit) == 0 && compatible[Integer.bitCount(mask)][w] == 1) {
                    int newmask = mask | wbit;
                    ways[newmask] += ways[mask];
                    ways[newmask] %= MOD;
                }
            }
        }
