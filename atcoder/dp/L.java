...
public class Main implements Runnable {
    ...
    private static long getDiff(int[] a) {
        long[][] diffs = new long[a.length][a.length];

        /*
            diffs{i, j} depends on : diffs{i+1, j} and diffs{i, j-1}
            -> process i in decreasing order and j in increasing order
        */

        for (int start = a.length-1; start >= 0; start--) {
            for (int end = start; end < a.length; end++) {
                int player = ((start + end) ^ a.length) & 1;
                if (player == 1) {
                    if (start == end) {
                        diffs[start][end] = a[start];
                    } else {
                        diffs[start][end] = Math.max(diffs[start+1][end] + a[start], diffs[start][end-1] + a[end]);
                    }
                } else {
                    if (start == end) {
                        diffs[start][end] = -a[start];
                    } else {
                        diffs[start][end] = Math.min(diffs[start+1][end] - a[start], diffs[start][end-1] - a[end]);
                    }
                }
            }
        }

        return diffs[0][a.length-1];
    }
}