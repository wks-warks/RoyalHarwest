...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        double[] headProbability = in.nextDoubleArray(coins);

        double moreHeadProbability = computeMoreHeadProbability(coins, headProbability);
        out.println(moreHeadProbability);
        
        out.close();
    }

    private static double computeMoreHeadProbability(int coins, double[] headProbability) {
        double[] headCountProbability = new double[coins+1];
        headCountProbability[0] = 1.0;

        for (var prob : headProbability) {
            for (int heads = coins; heads > 0; heads--) {
                headCountProbability[heads] = headCountProbability[heads] * (1.0 - prob) + headCountProbability[heads-1] * prob;
            }
            headCountProbability[0] *= 1.0 - prob;
        }

        int half = (coins+1)>>1;
        double netProbability = 0.0;
        for (int heads = half; heads <= coins; heads++) {
            netProbability += headCountProbability[heads];
        }
