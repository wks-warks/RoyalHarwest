...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int longestPathLength = graph.getLongestPathLength();
        out.println(longestPathLength);
...
class Graph {
    ...
    void addEdge(int from, int to) {
        nodes[to].addReachableFrom(nodes[from]);
    }

    int getLongestPathLength() {
        int longestPathLength = 0;
        for (var node : nodes) {
            longestPathLength = Math.max(longestPathLength, node.getLongestPathLength());
        }
...
class Node {
    private int idx;
    private List<Node> reachableFrom;
    private Integer longestPathLength;

    Node (int idx) {
        this.idx = idx;
        reachableFrom = new ArrayList<Node>();
        longestPathLength = null;
    }

    void addReachableFrom(Node nbr) {
        reachableFrom.add(nbr);
    }
    ...
    int getLongestPathLength() {
        if (longestPathLength != null) {
            ...
            int prevLength = -1;
            for (var nbr : reachableFrom) {
                prevLength = Math.max(prevLength, nbr.getLongestPathLength());
            }
            return longestPathLength = prevLength + 1;
        }
    }
}