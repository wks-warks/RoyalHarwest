...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        long maximumValue = computeMaximumValue(capacity, itemDetails);
        out.println(maximumValue);
        
        out.close();
    }

    private static long computeMaximumValue(int capacity, int[][] itemDetails) {
        long[] maxValue = new long[capacity+1];

        for (var details : itemDetails) {
            int weight = details[0];
            int value = details[1];
            
            for (int netWeight = capacity; netWeight >= weight; netWeight--) {
                long candidate = maxValue[netWeight-weight] + value;
                maxValue[netWeight] = Math.max(maxValue[netWeight], candidate);
            }
        }
