...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[] heights = in.nextIntArray(count);
        int[] beauties = in.nextIntArray(count);

        long maxBeauty = getMaxBeauty(count, heights, beauties);
        out.println(maxBeauty);
        
        out.close();
    }

    private static long getMaxBeauty(int count, int[] heights, int[] beauties) {
        SegmentTree st = new SegmentTree(count);

        for (int f = count - 1; f >= 0; f--) {
            st.addFlower(heights[f], beauties[f]);
        }

        return st.getMaxBeauty();
    }
}
...
class SegmentTree {
    ...
    SegmentTree(int count) {
        root = new Node(0, count);
    }

    void addFlower(int height, int beauty) {
        long addition = root.getMax(height + 1, root.right);
        root.setMax(height, height, addition + beauty);
    }

    long getMaxBeauty() {
        return root.getMax(root.left, root.right);
    }
}
...
class Node {
    ...
    Node(int left, int right, long max) {
        this.left = left;
        this.right = right;
    ...
    void setMax(int left, int right, long value) {
        ...
        if (this.left == left && this.right == right) {
            if (value > max) {
                ...
                if (!all) {
                    all = true;
                    leftChild.setMax(left, mid, value);
                    rightChild.setMax(mid + 1, right, value);
                }
            }
            return;
        }
        ...
        if (left > mid) {
            rightChild.setMax(left, right, value);
        } else if (right <= mid) {
            leftChild.setMax(left, right, value);
        } else {
            leftChild.setMax(left, mid, value);
            rightChild.setMax(mid + 1, right, value);
        }
    ...
    long getMax(int left, int right) {
        ...
        if (left > mid) {
            return rightChild.getMax(left, right);
        } else if (right <= mid) {
            return leftChild.getMax(left, right);
        } else {
            return Math.max(leftChild.getMax(left, mid), rightChild.getMax(mid + 1, right));
        }
    }
}