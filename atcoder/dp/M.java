...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[] bounds = in.nextIntArray(children);

        int ways = countWays(candies, bounds);
        out.println(ways);
    ...
    private static int countWays(int candies, int[] bounds) {
        int[] suffixSum = new int[candies+2];
        for (int c = 0; c <= candies; c++) {
            suffixSum[c] = 1;
        }

        for (var bound : bounds) {
            for (int rem = 0; rem <= candies; rem++) {
                suffixSum[rem] = suffixSum[rem] - suffixSum[Math.min(rem+bound, candies)+1];
                suffixSum[rem] %= MOD;
                suffixSum[rem] += MOD;
                suffixSum[rem] %= MOD;
            }

            for (int c = candies; c >= 0; c--) {
                suffixSum[c] += suffixSum[c+1];
                suffixSum[c] %= MOD;
            }
        }

        return (suffixSum[0] - suffixSum[1] + MOD) % MOD;
    }
