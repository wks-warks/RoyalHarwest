...
fn main() {
    ...
    let min_diff = get_min_diff(h, w);
    writeln!(out, "{}", min_diff).ok();
}

fn get_min_diff(h: u64, w: u64) -> u64 {
    let (min, max) = (std::cmp::min(h, w), std::cmp::max(h, w));
    let mut diff = std::u64::MAX;
    ...
    for i in 1..min {
        ...
        let candidate = std::cmp::max(a1, std::cmp::max(a2, a3)) - std::cmp::min(a1, std::cmp::min(a2, a3));
        diff = std::cmp::min(diff, candidate);
