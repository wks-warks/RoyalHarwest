...
fn get_ans(k: usize, a: Vec<u32>) -> isize {
    let mut map = BTreeMap::new();
    for i in (k..a.len()).rev() {
        map.insert(a[i], i);
    }

    let mut prev_best = std::usize::MAX;
    for map_entry in map.iter_mut().rev() {
        *map_entry.1 = min(*map_entry.1, prev_best);
        prev_best = *map_entry.1
    }

    let mut ans = -1;
    for i in 0..k {
        let higher = a[i] + 1;
        if let Some(successor) = map.range(higher..).next() {
            let j = successor.1;
            let swaps = (j - i) as isize;
