...
fn main() {
    ...
    for _ in 0..n {
        a.push_back(scan.next_u8());
    }

    let possible = is_possible(a);
    writeln!(out, "{}", if possible { "Yes" } else { "No" }).ok();
}

fn is_possible(mut a: VecDeque<u8>) -> bool {
    let mut flip = false;

    while !a.is_empty() {
        while let Some(back) = a.back() {
            if *back == effective_zero(flip) {
                a.pop_back();
            } else {
                break;
            }
        ...
        if let Some(front) = a.front() {
            if *front == effective_zero(flip) {
                a.pop_front();
                flip = !flip;
            } else {
                return false;
...
fn effective_zero(flip: bool) -> u8 {
    if flip { 1 } else { 0 }
}
