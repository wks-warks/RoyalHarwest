...
fn main() {
    ...
    let (a, b, c, x) = (scan.next_u16(), scan.next_u16(), scan.next_u16(), scan.next_u16());
    let prob = get_prob(a, b, c, x);
    writeln!(out, "{}", prob).ok();
}

fn get_prob(a: u16, b: u16, c: u16, x: u16) -> f32 {
    if x > b {
        return 0.0;
    } else if x > a {
        return (c as f32) / (b as f32 - a as f32);
    } else {
        return 1.0;
    }
}
