...
public class Main implements Runnable {
    ...
    public void run() {
        int[][] posMatrix = in.nextIntMatrix(in.nextInt(), 2);

        double maxDist = getMaxDist(posMatrix);
        out.println(maxDist);
        
        out.close();
    }

    private static double getMaxDist(int[][] posMatrix) {
        double maxDist = 0.0;

        for (int i = 0; i < posMatrix.length; i++) {
            for (int j = i+1; j < posMatrix.length; j++) {
                maxDist = Math.max(maxDist, dist(posMatrix[i], posMatrix[j]));
            }
        }
