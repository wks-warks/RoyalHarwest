...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        long arithmeticSuccessor = findArithmeticSuccessor(num);
        out.println(arithmeticSuccessor);
        
        out.close();
    }
    
    static TreeSet<Long> arithmeticSet;
    private static void fillArithmeticSet() {
        arithmeticSet = new TreeSet<Long>();

        for (int len = 1; len <= 18; len++) {
            for (int start = 9; start > 0; start--) {
                for (int diff = 9; diff >= 0; diff--) {
                    populate(len, start, diff);
                    populate(len, start, -diff);
                }
            }
        }
    ...
    private static void populate(int len, int start, int diff) {
        long num = 0;
        for (int i = 0; i < len; i++) {
            int digit = start - (diff * i);
            if (digit < 0 || digit > 9) {
                return;
            }
            num *= 10;
        ...
        arithmeticSet.add(num);
    }

    private static long findArithmeticSuccessor(long num) {
        long arithmeticSuccessor = arithmeticSet.ceiling(num);
        return arithmeticSuccessor;
    }
}