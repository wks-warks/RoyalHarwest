...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int[][] transitionMatrix = in.nextIntMatrix(c, c);
        int[][] colorMatrix = in.nextIntMatrix(n, n);
    ...
    private static int computeMinWrongness(int n, int c, int[][] transitionMatrix, int[][] colorMatrix) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                for (int k = 0; k < c; k++) {
                    wrongnessSum[mod3][k] += transitionMatrix[colorMatrix[i][j]][k];
                }
            }
        }

        int[][][] minWrongnessMatrix = new int[c][c][c];
        for (int i = 0; i < c; i++) {
            for (int j = 0; j < c; j++) {
                if (i == j) {
                    continue;
                ...
                for (int k = 0; k < c; k++) {
                    ...
                    minWrongnessMatrix[i][j][k] = wrongnessSum[0][i] + wrongnessSum[1][j] + wrongnessSum[2][k];
                }
            }
        }
        ...
        for (int i = 0; i < c; i++) {
            for (int j = 0; j < c; j++) {
                ...
                for (int k = 0; k < c; k++) {
                    ...
                    minWrongness = Math.min(minWrongness, minWrongnessMatrix[i][j][k]);
                }
            }
        }
        return minWrongness;
