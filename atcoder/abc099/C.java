...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int moves = computeMoves(n);
        out.println(moves);
        
        out.close();
    }

    private static int computeMoves(int n) {
        int[] minMoves = new int[n+1];
        for (int i = 1; i <= n; i++) {
            minMoves[i] = n;
        }

        List<Integer> sixPows = getPowsList(n, 6);
        List<Integer> ninePows = getPowsList(n, 9);

        for (int i = 0; i < n; i++) {
            minMoves[i + 1] = Math.min(minMoves[i + 1], minMoves[i] + 1);
            for (var sixPow : sixPows) {
                if (sixPow + i > n) {
                    break;
                }
                minMoves[i + sixPow] = Math.min(minMoves[i + sixPow], minMoves[i] + 1);
            }
            for (var ninePow : ninePows) {
                if (ninePow + i > n) {
                    break;
                }
                minMoves[i + ninePow] = Math.min(minMoves[i + ninePow], minMoves[i] + 1);
            }
        }
    ...
    private static List<Integer> getPowsList(int n, int base) {
        if (base < 2) {
            return null;
        }

        List<Integer> powsList = new ArrayList<>();
        int pow = base;
        while (pow <= n) {
            powsList.add(pow);
            pow *= base;
