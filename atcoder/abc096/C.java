...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        for (int r = 0; r < h; r++) {
            matrix[r] = in.next().toCharArray();
        }

        boolean colorable = isColorable(matrix);
        out.println(colorable ? "Yes" : "No");
    ...
    private static boolean isColorable(char[][] matrix) {
        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix[i].length; j++) {
                if (matrix[i][j] == '#') {
                    boolean colorableCell = false;
                    for (var nbr : neighbours) {
                        colorableCell |= validCell(i + nbr[0], j + nbr[1], matrix) && matrix[i + nbr[0]][j + nbr[1]] == '#';
                    }
    ...
    private static boolean validCell(int i, int j, char[][] matrix) {
        return Math.min(i, j) >= 0 && i < matrix.length && j < matrix[i].length;
    }
}
