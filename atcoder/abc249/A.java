...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int takahashi = getDistance(0, a, b, 0, c, x);
        int aoki = getDistance(0, d, e, 0, f, x);
        // out.println(takahashi + " " + aoki);

        out.println(takahashi > aoki ? "Takahashi" : aoki > takahashi ? "Aoki" : "Draw");
        
        out.close();
    }

    static int getDistance(int currTime, int currSpeed, int currDuration, int nextSpeed, int nextDuration, int endTime) {
        if (currTime + currDuration >= endTime) {
            return currSpeed * (endTime - currTime);
        } else {
            return currSpeed * currDuration + getDistance(currTime + currDuration, nextSpeed, nextDuration, currSpeed, currDuration, endTime);
        }
    }
}
