...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        long trios = countTrios(a);
        out.println(trios);

        out.close();
    }

    static long countTrios(int[] a) {
        Map<Integer, Integer> freq = new TreeMap<Integer, Integer>(Collections.reverseOrder());
        for (var num : a) {
        ...
        for (var me : freq.entrySet()) {
            ...
            int val = me.getValue();
            if (key == 1) {
                trios += (long) val * val * val;
                break;
            }

            for (int i = 1; i * i <= key; i++) {
                if (key % i == 0) {
                    if (freq.containsKey(i) && freq.containsKey(key / i)) {
                        trios += (long) val * freq.get(i) * freq.get(key / i);
                        if (i * i != key) {
                            trios += (long) val * freq.get(i) * freq.get(key / i);
                        }
                    }
