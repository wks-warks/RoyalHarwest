...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        List<List<Integer>> nbrs = new ArrayList<List<Integer>>();
        for (int i = 0; i < n; i++) {
        ...
        for (int len = 1; len < k; len++) {
            for (int nd = 0; nd < n; nd++) {
                for (int nbr : nbrs.get(nd)) {
                    int xor = (nd == x) ? 1 : 0;
                    dp[nbr][len + 1][1] += dp[nd][len][1 ^ xor];
                    dp[nbr][len + 1][0] += dp[nd][len][0 ^ xor];
                    dp[nbr][len + 1][0] %= MOD;
                    dp[nbr][len + 1][1] %= MOD;
                }
            }
        ...
        //         out.println("i: " + i + " k: " + len + " c: " + dp[i][len][0]);
        //     }
        // }

        out.println(dp[t][k][0]);
