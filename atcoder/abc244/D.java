...
public class Main implements Runnable {
    ...
    public void run() {
        char[] s = readCharArr(3);
        char[] t = readCharArr(3);

        int difference = getDifference(s, t);
        out.println((difference == 2) ? "No" : "Yes");
        
        out.close();
    }

    private static char[] readCharArr(int len) {
    ...
    private static int getDifference(char[] s, char[] t) {
        int difference = 0;
...
even: 0 or 3
s = s0 = perm(RGB, 0)
t = s1e18 = perm(RGB, 1e18)

s1 = perm(RGB, 1) -> 2; difference with s0 = 2
s2 = perm(RGB, 2) -> 4; difference with s0 = 0 or 
