...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        play(nums, nums.length, 0);
        
        out.close();
    }

    void play(int[] nums, int moves, int ptr) {
        if (moves == 0) {
            return;
        }

        if ((moves & 1) > 0) { // Takahashi
            ptr = takahashi(nums, ptr);
        } else { // Aoki
            aoki(nums);
        }

        play(nums, moves - 1, ptr);
    }

    int takahashi(int[] nums, int ptr) {
        while (nums[ptr] == -1) {
            ptr++;
        }

        out.println(nums[ptr]);
        out.flush();
        nums[ptr] = -1;
