...
public class Main implements Runnable {
    ...
    public void run() {
        ...
        int abPrice = in.nextInt();

        int aReqd = in.nextInt();
        int bReqd = in.nextInt();

        int minCost = getMinCost(aReqd, bReqd, aPrice, bPrice, abPrice);
        out.println(minCost);
        
        out.close();
    }

    private static int getMinCost(int aReqd, int bReqd, int aPrice, int bPrice, int abPrice) {
        int a, b, ab;
        a = b = ab = 0;
        if ((abPrice<<1) <= aPrice + bPrice) {
            ab = Math.min(aReqd, bReqd)<<1;
            aReqd -= ab>>1;
            bReqd -= ab>>1;
        }
        ...
        return a*aPrice + b*bPrice + ab*abPrice;
    }
}