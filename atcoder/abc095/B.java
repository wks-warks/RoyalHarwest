...
public class Main implements Runnable {
    ...
    public void run() {
        int doughnutTypes = in.nextInt();
        int dough = in.nextInt();
        int[] doughRequired = in.nextIntArray(doughnutTypes);

        int doughnutsMade = countDoughnutsMade(dough, doughRequired);
        out.println(doughnutsMade);
        
        out.close();
    }

    private static int countDoughnutsMade(int dough, int[] doughRequired) {
        int doughnutsMade = 0;
        int min = Integer.MAX_VALUE;
        
        for (var req : doughRequired) {
            min = Math.min(min, req);
            dough -= req;
            doughnutsMade++;
        }

        doughnutsMade += dough / min;
        return doughnutsMade;
    }
}